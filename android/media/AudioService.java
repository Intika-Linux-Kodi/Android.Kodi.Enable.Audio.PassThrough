...
public class AudioService extends Stub {
    ...
    class C04701 implements ErrorCallback {
        C04701() {
        }
    ...
    class C04712 implements ServiceListener {
        C04712() {
        }
                    ...
                    synchronized (AudioService.this.mA2dpAvrcpLock) {
                        ...
                        synchronized (AudioService.this.mConnectedDevices) {
                            if (AudioService.this.mConnectedDevices.containsKey(Integer.valueOf(AccessibilityNodeInfo.ACTION_CLEAR_ACCESSIBILITY_FOCUS))) {
                                AudioService.this.makeA2dpDeviceUnavailableNow((String) AudioService.this.mConnectedDevices.get(Integer.valueOf(AccessibilityNodeInfo.ACTION_CLEAR_ACCESSIBILITY_FOCUS)));
                            }
                            break;
                        }
                        break;
    ...
    private class AudioHandler extends Handler {
        ...
        class C04731 implements OnCompletionListener {
            C04731() {
            }
        ...
        class C04742 implements OnErrorListener {
            C04742() {
            }
        ...
        private void persistRingerMode(int ringerMode) {
            if (!AudioService.this.mUseFixedVolume) {
                Global.putInt(AudioService.this.mContentResolver, System.MODE_RINGER, ringerMode);
            }
        }
                ...
                if (!AudioService.this.mSystemReady) {
                    ...
                    while (AudioService.this.mSoundPoolCallBack == null) {
                        ...
                        try {
                            ...
                            Log.m8w(AudioService.TAG, "Interrupted while waiting sound pool listener thread.");
                            attempts2 = attempts;
                        }
                    }
                    attempts = attempts2;
                    if (AudioService.this.mSoundPoolCallBack == null) {
                        Log.m8w(AudioService.TAG, "onLoadSoundEffects() SoundPool listener or thread creation error");
                    ...
                    for (effect = AudioService.SENDMSG_REPLACE; effect < AudioService.MSG_SET_ALL_VOLUMES; effect += AudioService.SENDMSG_NOOP) {
                        if (AudioService.this.SOUND_EFFECT_FILES_MAP[effect][AudioService.SENDMSG_NOOP] != 0) {
                            if (poolId[AudioService.this.SOUND_EFFECT_FILES_MAP[effect][AudioService.SENDMSG_REPLACE]] == AudioService.SCO_MODE_UNDEFINED) {
                                ...
                                if (sampleId <= 0) {
                                    Log.m8w(AudioService.TAG, "Soundpool could not load file: " + filePath);
                                } else {
                    ...
                    if (numSamples > 0) {
                        ...
                        while (status == AudioService.SENDMSG_NOOP) {
                            ...
                            try {
                                ...
                                Log.m8w(AudioService.TAG, "Interrupted while waiting sound pool callback.");
                                attempts2 = attempts;
                            }
                        }
                    } else {
                    ...
                    AudioService.this.mSoundPoolListenerThread = null;
                    if (status != 0) {
                        Log.m8w(AudioService.TAG, "onLoadSoundEffects(), Error " + status + " while loading samples");
                        for (effect = AudioService.SENDMSG_REPLACE; effect < AudioService.MSG_SET_ALL_VOLUMES; effect += AudioService.SENDMSG_NOOP) {
                            if (AudioService.this.SOUND_EFFECT_FILES_MAP[effect][AudioService.SENDMSG_NOOP] > 0) {
            ...
            synchronized (AudioService.this.mSoundEffectsLock) {
                ...
                if (AudioService.this.SOUND_EFFECT_FILES_MAP[effectType][AudioService.SENDMSG_NOOP] > 0) {
                    ...
                    try {
                        ...
                        mediaPlayer.setOnCompletionListener(new C04731());
                        mediaPlayer.setOnErrorListener(new C04742());
                        mediaPlayer.start();
                    } catch (IOException ex) {
                        Log.m8w(AudioService.TAG, "MediaPlayer IOException: " + ex);
                    } catch (IllegalArgumentException ex2) {
                        Log.m8w(AudioService.TAG, "MediaPlayer IllegalArgumentException: " + ex2);
                    } catch (IllegalStateException ex3) {
                        Log.m8w(AudioService.TAG, "MediaPlayer IllegalStateException: " + ex3);
                    }
                }
            }
        }
        ...
        private void cleanupPlayer(MediaPlayer mp) {
            if (mp != null) {
                try {
                    ...
                    Log.m8w(AudioService.TAG, "MediaPlayer IllegalStateException: " + ex);
                }
            }
        }
            ...
            switch (msg.what) {
                ...
                case AudioService.SCO_STATE_DEACTIVATE_EXT_REQ /*4*/:
                    if (AudioService.this.mSystemReady && AudioSystem.checkAudioFlinger() == 0) {
                        Log.m2e(AudioService.TAG, "Media server started.");
                    ...
                    Log.m2e(AudioService.TAG, "Media server died.");
    ...
    public class AudioPolicyProxy extends AudioPolicyConfig implements DeathRecipient {
        ...
        public void binderDied() {
            synchronized (AudioService.this.mAudioPolicies) {
                Log.m4i(TAG, "audio policy " + this.mPolicyToken + " died");
                release();
    ...
    private class ForceControlStreamClient implements DeathRecipient {
        ...
        ForceControlStreamClient(IBinder cb) {
            if (cb != null) {
                try {
                    ...
                    Log.m8w(AudioService.TAG, "ForceControlStreamClient() could not link to " + cb + " binder death");
                    cb = null;
                }
            }
            this.mCb = cb;
        ...
        public void binderDied() {
            synchronized (AudioService.this.mForceControlStreamLock) {
                Log.m8w(AudioService.TAG, "SCO client died");
                if (AudioService.this.mForceControlStreamClient != this) {
                    Log.m8w(AudioService.TAG, "unregistered control stream client died");
                } else {
    ...
    private class RmtSbmxFullVolDeathHandler implements DeathRecipient {
        ...
        RmtSbmxFullVolDeathHandler(IBinder cb) {
            ...
            try {
                ...
                Log.m3e(AudioService.TAG, "can't link to death", e);
            }
        }
        ...
        void forget() {
            try {
                ...
                Log.m3e(AudioService.TAG, "error unlinking to death", e);
            }
        }

        public void binderDied() {
            Log.m8w(AudioService.TAG, "Recorder with remote submix at full volume died " + this.mICallback);
    ...
    private class ScoClient implements DeathRecipient {
        ...
        public void binderDied() {
            synchronized (AudioService.this.mScoClients) {
                ...
                if (AudioService.this.mScoClients.indexOf(this) < 0) {
                    Log.m8w(AudioService.TAG, "unregistered SCO client died");
                } else {
            ...
            synchronized (AudioService.this.mScoClients) {
                ...
                if (this.mStartcount == 0) {
                    try {
                        ...
                        Log.m8w(AudioService.TAG, "ScoClient  incCount() could not link to " + this.mCb + " binder death");
                    }
                }
        ...
        public void decCount() {
            synchronized (AudioService.this.mScoClients) {
                if (this.mStartcount == 0) {
                    Log.m8w(AudioService.TAG, "ScoClient.decCount() already 0");
                } else {
                    ...
                    if (this.mStartcount == 0) {
                        try {
                            ...
                            Log.m8w(AudioService.TAG, "decCount() going to 0 but not registered to binder");
                        }
                    }
            ...
            synchronized (AudioService.this.mScoClients) {
                if (this.mStartcount != 0) {
                    try {
                        ...
                        Log.m8w(AudioService.TAG, "clearCount() mStartcount: " + this.mStartcount + " != 0 but not registered to binder");
                    }
                }
                this.mStartcount = AudioService.SENDMSG_REPLACE;
                if (stopSco) {
    ...
    private class SetModeDeathHandler implements DeathRecipient {
        ...
        public void binderDied() {
            ...
            synchronized (AudioService.this.mSetModeDeathHandlers) {
                Log.m8w(AudioService.TAG, "setMode() client died");
                if (AudioService.this.mSetModeDeathHandlers.indexOf(this) < 0) {
                    Log.m8w(AudioService.TAG, "unregistered setMode() client died");
                } else {
    ...
    private class SettingsObserver extends ContentObserver {
        SettingsObserver() {
            super(new Handler());
            AudioService.this.mContentResolver.registerContentObserver(System.getUriFor(System.MODE_RINGER_STREAMS_AFFECTED), AudioService.DEBUG_VOL, this);
            AudioService.this.mContentResolver.registerContentObserver(Global.getUriFor(Global.DOCK_AUDIO_MEDIA_ENABLED), AudioService.DEBUG_VOL, this);
        }
    ...
    private static class StreamOverride implements TouchExplorationStateChangeListener {
        ...
        private static void updateDefaultStreamOverrideDelay(boolean touchExploreEnabled) {
            ...
            if (AudioService.DEBUG_VOL) {
                Log.m0d(AudioService.TAG, "Touch exploration enabled=" + touchExploreEnabled + " stream override delay is now " + sDelayMs + " ms");
            }
        }
    }
    ...
    public static class VolumeController {
        ...
        public void loadSettings(ContentResolver cr) {
            this.mLongPressTimeout = Secure.getIntForUser(cr, Secure.LONG_PRESS_TIMEOUT, AudioService.PERSIST_DELAY, -2);
        }
        ...
        public void postDisplaySafeVolumeWarning(int flags) {
            if (this.mController != null) {
                try {
                    ...
                    Log.m9w(TAG, "Error calling displaySafeVolumeWarning", e);
                }
            }
        }
        ...
        public void postVolumeChanged(int streamType, int flags) {
            if (this.mController != null) {
                try {
                    ...
                    Log.m9w(TAG, "Error calling volumeChanged", e);
                }
            }
        }
        ...
        public void postMasterVolumeChanged(int flags) {
            if (this.mController != null) {
                try {
                    ...
                    Log.m9w(TAG, "Error calling masterVolumeChanged", e);
                }
            }
        }
        ...
        public void postMasterMuteChanged(int flags) {
            if (this.mController != null) {
                try {
                    ...
                    Log.m9w(TAG, "Error calling masterMuteChanged", e);
                }
            }
        }
        ...
        public void setLayoutDirection(int layoutDirection) {
            if (this.mController != null) {
                try {
                    ...
                    Log.m9w(TAG, "Error calling setLayoutDirection", e);
                }
            }
        }
        ...
        public void postDismiss() {
            if (this.mController != null) {
                try {
                    ...
                    Log.m9w(TAG, "Error calling dismiss", e);
                }
            }
        }
    }
    ...
    public class VolumeStreamState {
        ...
        private class VolumeDeathHandler implements DeathRecipient {
            ...
            public void mute_syncVSS(boolean state) {
                ...
                if (state) {
                    ...
                    Log.m8w(AudioService.TAG, "stream: " + VolumeStreamState.this.mStreamType + " was already muted by this client");
                    this.mMuteCount += AudioService.SENDMSG_NOOP;
                } else if (this.mMuteCount == 0) {
                    Log.m2e(AudioService.TAG, "unexpected unmute for stream: " + VolumeStreamState.this.mStreamType);
                } else {
            ...
            public void binderDied() {
                Log.m8w(AudioService.TAG, "Volume service client died for stream: " + VolumeStreamState.this.mStreamType);
                synchronized (VolumeStreamState.class) {
                    if (this.mMuteCount != 0) {
                        this.mMuteCount = AudioService.SENDMSG_NOOP;
        ...
        public void mute(IBinder cb, boolean state) {
            synchronized (VolumeStreamState.class) {
                ...
                if (handler == null) {
                    Log.m2e(AudioService.TAG, "Could not get client death handler for stream: " + this.mStreamType);
                    return;
                }
        ...
        private VolumeDeathHandler getDeathHandler_syncVSS(IBinder cb, boolean state) {
            ...
            if (state) {
                ...
                Log.m8w(AudioService.TAG, "stream was not muted by this client");
                handler = null;
            }
            return handler;
        }
    ...
    public AudioService(Context context) {
        ...
        this.mAudioSystemCallback = new C04701();
        ...
        this.mFixedVolumeDevices = SENDMSG_REPLACE;
        ...
        this.mBluetoothProfileServiceListener = new C04712();
        ...
        if (this.mContext.getResources().getBoolean(17956942)) {
        ...
        boolean cameraSoundForced = this.mContext.getResources().getBoolean(17956975);
        ...
        this.mSafeMediaVolumeState = new Integer(Global.getInt(this.mContentResolver, Global.AUDIO_SAFE_VOLUME_STATE, SENDMSG_REPLACE));
        this.mSafeMediaVolumeIndex = this.mContext.getResources().getInteger(17694838) * MSG_SET_ALL_VOLUMES;
        this.mUseFixedVolume = this.mContext.getResources().getBoolean(17956980);
        this.mUseMasterVolume = context.getResources().getBoolean(17956879);
        ...
        if (this.mMonitorOrientation) {
            Log.m6v(TAG, "monitoring device orientation");
        ...
        if (this.mMonitorRotation) {
            ...
            Log.m6v(TAG, "monitoring device rotation, initial=" + this.mDeviceRotation);
    ...
    L_0x0009:
        ...
        android.util.Log.m2e(r1, r2);	 Catch:{ all -> 0x0012 }
        goto L_0x0001;
    L_0x0012:
        r1 = move-exception;
    ...
    private void readDockAudioSettings(ContentResolver cr) {
        ...
        if (Global.getInt(cr, Global.DOCK_AUDIO_MEDIA_ENABLED, SENDMSG_REPLACE) != SENDMSG_NOOP) {
            z = DEBUG_VOL;
        }
    ...
    private void readPersistedSettings() {
        ...
        int ringerModeFromSettings = Global.getInt(cr, System.MODE_RINGER, SENDMSG_QUEUE);
        ...
        if (ringerMode != ringerModeFromSettings) {
            Global.putInt(cr, System.MODE_RINGER, ringerMode);
        }
        ...
        this.mMuteAffectedStreams = System.getIntForUser(cr, System.MUTE_STREAMS_AFFECTED, MSG_CHECK_MUSIC_ACTIVE, -2);
        if (System.getIntForUser(cr, System.VOLUME_MASTER_MUTE, SENDMSG_REPLACE, -2) == SENDMSG_NOOP) {
        ...
        if (System.getIntForUser(cr, System.MICROPHONE_MUTE, SENDMSG_REPLACE, -2) == SENDMSG_NOOP) {
    ...
    private void adjustSuggestedStreamVolume(int direction, int suggestedStreamType, int flags, String callingPackage, int uid) {
        int streamType;
        if (DEBUG_VOL) {
            Log.m0d(TAG, "adjustSuggestedStreamVolume() stream=" + suggestedStreamType + ", flags=" + flags);
        }
        ...
        if (this.mVolumeController.suppressAdjustment(resolvedStream, flags)) {
            ...
            flags = (flags & -5) & -17;
            if (DEBUG_VOL) {
                Log.m0d(TAG, "Volume controller suppressed adjustment");
            }
        }
    ...
    private void adjustStreamVolume(int streamType, int direction, int flags, String callingPackage, int uid) {
        if (!this.mUseFixedVolume) {
            if (DEBUG_VOL) {
                Log.m0d(TAG, "adjustStreamVolume() stream=" + streamType + ", dir=" + direction + ", flags=" + flags);
            }
                ...
                if (adjustVolume && direction != 0) {
                    ...
                    if (direction == SENDMSG_NOOP) {
                        if (!checkSafeMediaVolume(streamTypeAlias, aliasIndex + step, device)) {
                            Log.m2e(TAG, "adjustStreamVolume() safe volume index = " + oldIndex);
    ...
    public void forceRemoteSubmixFullVolume(boolean startForcing, IBinder cb) {
        if (cb != null) {
            if (this.mContext.checkCallingOrSelfPermission(permission.CAPTURE_AUDIO_OUTPUT) != 0) {
                Log.m8w(TAG, "Trying to call forceRemoteSubmixFullVolume() without CAPTURE_AUDIO_OUTPUT");
                return;
            }
    ...
    public void setMode(int mode, IBinder cb) {
        if (DEBUG_MODE) {
            Log.m6v(TAG, "setMode(mode=" + mode + ")");
        }
        if (!checkAudioSettingsPermission("setMode()")) {
            return;
        }
        if (mode == SENDMSG_QUEUE && this.mContext.checkCallingOrSelfPermission(permission.MODIFY_PHONE_STATE) != 0) {
            Log.m8w(TAG, "MODIFY_PHONE_STATE Permission Denial: setMode(MODE_IN_CALL) from pid=" + Binder.getCallingPid() + ", uid=" + Binder.getCallingUid());
    ...
    private int setModeInt(int mode, IBinder cb, int pid) {
        if (DEBUG_MODE) {
            Log.m6v(TAG, "setModeInt(mode=" + mode + ", pid=" + pid + ")");
        }
        int newModeOwnerPid = SENDMSG_REPLACE;
        if (cb == null) {
            Log.m2e(TAG, "setModeInt() called with null binder");
            return SENDMSG_REPLACE;
        }
        ...
        do {
            ...
            if (mode != 0) {
                ...
                try {
                    ...
                    Log.m8w(TAG, "setMode() could not link to " + cb + " binder death");
                }
                ...
                mode = hdlr.getMode();
                if (DEBUG_MODE) {
                    Log.m8w(TAG, " using mode=" + mode + " instead due to death hdlr at pid=" + hdlr.mPid);
                }
            }
            if (mode != this.mMode) {
                ...
                if (status == 0) {
                    if (DEBUG_MODE) {
                        Log.m6v(TAG, " mode successfully set to " + mode);
                    }
                    this.mMode = mode;
                } else {
                    if (hdlr != null) {
                    ...
                    if (DEBUG_MODE) {
                        Log.m8w(TAG, " mode set to MODE_NORMAL after phoneState pb");
                    }
        ...
        if (status == 0) {
            if (mode != 0) {
                if (this.mSetModeDeathHandlers.isEmpty()) {
                    Log.m2e(TAG, "setMode() different from MODE_NORMAL with empty mode client stack");
                } else {
    ...
    L_0x00a2:
        ...
        android.util.Log.m9w(r11, r12, r0);	 Catch:{ all -> 0x00ea }
        if (r9 == 0) goto L_0x0009;
    L_0x00ac:
        r9.close();
        goto L_0x0009;
    L_0x00b1:
        ...
        android.util.Log.m8w(r11, r12);	 Catch:{ NotFoundException -> 0x00a2, XmlPullParserException -> 0x00cc, IOException -> 0x00db }
        goto L_0x0039;
    L_0x00cc:
        r0 = move-exception;
        r11 = "AudioService";
    ...
    public void playSoundEffectVolume(int effectType, float volume) {
        if (!SystemProperties.getBoolean("media.cfg.audio.bypass", DEBUG_VOL)) {
            if (effectType >= MSG_SET_ALL_VOLUMES || effectType < 0) {
                Log.m8w(TAG, "AudioService effectType value " + effectType + " out of range");
                return;
            }
    ...
    L_0x0023:
        ...
        android.util.Log.m8w(r0, r1);	 Catch:{ all -> 0x0035 }
        r8 = r7;
        goto L_0x0014;
    L_0x002d:
        r7 = r8;
    ...
    private void readAudioSettings(boolean userSwitch) {
        ...
        checkAllAliasStreamVolumes();
        synchronized (this.mSafeMediaVolumeState) {
            this.mMusicActiveMs = MathUtils.constrain(Secure.getIntForUser(this.mContentResolver, Secure.UNSAFE_VOLUME_MUSIC_ACTIVE_MS, SENDMSG_REPLACE, -2), (int) SENDMSG_REPLACE, (int) UNSAFE_VOLUME_MUSIC_ACTIVE_MS_MAX);
    ...
    private void onConfigureSafeVolume(boolean force) {
        ...
        synchronized (this.mSafeMediaVolumeState) {
            ...
            if (this.mMcc != mcc || (this.mMcc == 0 && force)) {
                int persistedState;
                this.mSafeMediaVolumeIndex = this.mContext.getResources().getInteger(17694838) * MSG_SET_ALL_VOLUMES;
                if (SystemProperties.getBoolean("audio.safemedia.force", DEBUG_VOL) || this.mContext.getResources().getBoolean(17956973)) {
    ...
    private int checkForRingerModeChange(int oldIndex, int direction, int step) {
        ...
        switch (ringerMode) {
            ...
            case SENDMSG_NOOP /*1*/:
                if (!this.mHasVibrator) {
                    Log.m2e(TAG, "checkForRingerModeChange() current ringer mode is vibratebut no vibrator is present");
                    break;
                }
                if (direction == SCO_MODE_UNDEFINED) {
            ...
            case SENDMSG_QUEUE /*2*/:
                ...
                break;
            default:
                Log.m2e(TAG, "checkForRingerModeChange() wrong ringer mode: " + ringerMode);
                break;
        }
    ...
    boolean updateRingerModeAffectedStreams() {
        int ringerModeAffectedStreams = System.getIntForUser(this.mContentResolver, System.MODE_RINGER_STREAMS_AFFECTED, KeyEvent.KEYCODE_CHANNEL_UP, -2) | 38;
        switch (this.mPlatformType) {
            case SENDMSG_QUEUE /*2*/:
    ...
    private int getActiveStreamType(int suggestedStreamType) {
        switch (this.mPlatformType) {
            case SENDMSG_NOOP /*1*/:
                if (isInCommunication()) {
                    ...
                    if (isAfMusicActiveRecently(StreamOverride.sDelayMs)) {
                        if (DEBUG_VOL) {
                            Log.m6v(TAG, "getActiveStreamType: Forcing STREAM_MUSIC stream active");
                        }
                        return SCO_STATE_ACTIVE_INTERNAL;
                    }
                    if (DEBUG_VOL) {
                        Log.m6v(TAG, "getActiveStreamType: Forcing STREAM_RING b/c default");
                    }
                    return SCO_STATE_ACTIVE_INTERNAL;
            ...
            default:
                if (isInCommunication()) {
                    if (AudioSystem.getForceUse(SENDMSG_REPLACE) == SCO_STATE_ACTIVE_INTERNAL) {
                        if (DEBUG_VOL) {
                            Log.m6v(TAG, "getActiveStreamType: Forcing STREAM_BLUETOOTH_SCO");
                        }
                        return MSG_BTA2DP_DOCK_TIMEOUT;
                    }
                    if (DEBUG_VOL) {
                        Log.m6v(TAG, "getActiveStreamType: Forcing STREAM_VOICE_CALL");
                    }
                    return SCO_STATE_ACTIVE_INTERNAL;
                    ...
                    if (DEBUG_VOL) {
                        Log.m6v(TAG, "getActiveStreamType: Forcing STREAM_NOTIFICATION");
                    }
                    return SCO_STATE_ACTIVE_INTERNAL;
                } else if (suggestedStreamType == RtlSpacingHelper.UNDEFINED) {
                    if (isAfMusicActiveRecently(StreamOverride.sDelayMs)) {
                        if (DEBUG_VOL) {
                            Log.m6v(TAG, "getActiveStreamType: forcing STREAM_MUSIC");
                        }
                        return SCO_STATE_ACTIVE_INTERNAL;
                    }
                    if (DEBUG_VOL) {
                        Log.m6v(TAG, "getActiveStreamType: using STREAM_NOTIFICATION as default");
                    }
                    return SCO_STATE_ACTIVE_INTERNAL;
                }
                break;
        ...
        Log.m6v(TAG, "getActiveStreamType: Returning suggested type " + suggestedStreamType);
        return suggestedStreamType;
    }
    ...
    boolean checkAudioSettingsPermission(String method) {
        if (this.mContext.checkCallingOrSelfPermission(permission.MODIFY_AUDIO_SETTINGS) == 0) {
            return PREVENT_VOLUME_ADJUSTMENT_IF_SILENT;
        }
        Log.m8w(TAG, "Audio Settings Permission Denial: " + method + " from pid=" + Binder.getCallingPid() + ", uid=" + Binder.getCallingUid());
        return DEBUG_VOL;
    }
    ...
    private int getDeviceForStream(int stream) {
        ...
        if ((device & AccessibilityNodeInfo.ACTION_NEXT_HTML_ELEMENT) != 0) {
    ...
    private void onSetA2dpSinkConnectionState(BluetoothDevice btDevice, int state) {
        ...
        if (DEBUG_VOL) {
            Log.m0d(TAG, "onSetA2dpSinkConnectionState btDevice=" + btDevice + "state=" + state);
        }
        if (btDevice != null) {
    ...
    private void onSetA2dpSourceConnectionState(BluetoothDevice btDevice, int state) {
        if (DEBUG_VOL) {
            Log.m0d(TAG, "onSetA2dpSourceConnectionState btDevice=" + btDevice + " state=" + state);
        }
        if (btDevice != null) {
            String address = btDevice.getAddress();
            if (!BluetoothAdapter.checkBluetoothAddress(address)) {
    ...
    private boolean handleDeviceConnection(boolean connected, int device, String params) {
        synchronized (this.mConnectedDevices) {
            ...
            if (device == AccessibilityNodeInfo.ACTION_COLLAPSE || device == AccessibilityNodeInfo.ACTION_NEXT_HTML_ELEMENT) {
                isSpdifOrHdmi = PREVENT_VOLUME_ADJUSTMENT_IF_SILENT;
            } else {
                isSpdifOrHdmi = DEBUG_VOL;
            }
            if (isSpdifOrHdmi) {
                if (connected) {
    ...
    private void sendDeviceConnectionIntent(int device, int state, String name) {
        ...
        } else if (device == 2048 || device == 524288) {
            ...
            if (!"AudioFocus_For_Phone_Ring_And_Calls".equals(clientId)) {
                synchronized (this.mAudioPolicies) {
                    if (this.mAudioPolicies.containsKey(pcb.asBinder())) {
                        ...
                        Log.m2e(TAG, "Invalid unregistered AudioPolicy to (un)lock audio focus");
                        return SENDMSG_REPLACE;
                    }
                }
            } else if (this.mContext.checkCallingOrSelfPermission(permission.MODIFY_PHONE_STATE) != 0) {
                Log.m3e(TAG, "Invalid permission to (un)lock audio focus", new Exception());
                return SENDMSG_REPLACE;
            }
        }
    ...
    private void handleConfigurationChanged(Context context) {
        try {
            ...
            Log.m3e(TAG, "Error handling configuration change: ", e);
        }
    }
    ...
    private void setOrientationForAudioSystem() {
        switch (this.mDeviceOrientation) {
            ...
            default:
                Log.m2e(TAG, "Unknown orientation");
        }
    }
    ...
    private void setRotationForAudioSystem() {
        switch (this.mDeviceRotation) {
            ...
            default:
                Log.m2e(TAG, "Unknown device rotation");
        }
    }
    ...
    public void setRingtonePlayer(IRingtonePlayer player) {
        this.mContext.enforceCallingOrSelfPermission(permission.REMOTE_AUDIO_PLAYBACK, null);
    ...
    public int setHdmiSystemAudioSupported(boolean on) {
        ...
        if (this.mHdmiManager != null) {
            synchronized (this.mHdmiManager) {
                if (this.mHdmiTvClient == null) {
                    Log.m8w(TAG, "Only Hdmi-Cec enabled TV device supports system audio mode.");
                    return SENDMSG_REPLACE;
                }
                synchronized (this.mHdmiTvClient) {
    ...
    private static void readAndSetLowRamDevice() {
        ...
        if (status != 0) {
            Log.m8w(TAG, "AudioFlinger informed of device's low RAM attribute; status " + status);
        }
    }

    private void enforceSelfOrSystemUI(String action) {
        this.mContext.enforceCallingOrSelfPermission(permission.STATUS_BAR_SERVICE, "Only SystemUI can " + action);
    }
    ...
    public void setVolumeController(IVolumeController controller) {
        ...
        if (!this.mVolumeController.isSameBinder(controller)) {
            ...
            if (controller != null) {
                try {
                    controller.asBinder().linkToDeath(new C04723(controller), SENDMSG_REPLACE);
                } catch (RemoteException e) {
                }
            }
            this.mVolumeController.setController(controller);
            if (DEBUG_VOL) {
                Log.m0d(TAG, "Volume controller: " + this.mVolumeController);
            }
        }
    }
    ...
    public void notifyVolumeControllerVisible(IVolumeController controller, boolean visible) {
        ...
        if (this.mVolumeController.isSameBinder(controller)) {
            ...
            if (DEBUG_VOL) {
                Log.m0d(TAG, "Volume controller visible: " + visible);
            }
        }
    }
    ...
    public String registerAudioPolicy(AudioPolicyConfig policyConfig, IAudioPolicyCallback pcb, boolean hasFocusListener) {
        ...
        if (DEBUG_AP) {
            Log.m0d(TAG, "registerAudioPolicy for " + pcb.asBinder() + " with config:" + policyConfig);
        }
        if (this.mContext.checkCallingPermission(permission.MODIFY_AUDIO_ROUTING) == 0) {
        ...
        if (hasPermissionForPolicy) {
            synchronized (this.mAudioPolicies) {
                try {
                    if (this.mAudioPolicies.containsKey(pcb.asBinder())) {
                        Slog.m13e(TAG, "Cannot re-register policy");
                        return null;
                    }
                    ...
                    Slog.m20w(TAG, "Audio policy registration failed, could not link to " + pcb + " binder death", e);
                    return null;
                }
            }
        }
        Slog.m19w(TAG, "Can't register audio policy for pid " + Binder.getCallingPid() + " / uid " + Binder.getCallingUid() + ", need MODIFY_AUDIO_ROUTING");
        return null;
    }

    public void unregisterAudioPolicyAsync(IAudioPolicyCallback pcb) {
        if (DEBUG_AP) {
            Log.m0d(TAG, "unregisterAudioPolicyAsync for " + pcb.asBinder());
        }
        synchronized (this.mAudioPolicies) {
            AudioPolicyProxy app = (AudioPolicyProxy) this.mAudioPolicies.remove(pcb.asBinder());
            if (app == null) {
                Slog.m19w(TAG, "Trying to unregister unknown audio policy for pid " + Binder.getCallingPid() + " / uid " + Binder.getCallingUid());
                return;
            }
    ...
    public int setFocusPropertiesForPolicy(int duckingBehavior, IAudioPolicyCallback pcb) {
        ...
        if (DEBUG_AP) {
            Log.m0d(TAG, "setFocusPropertiesForPolicy() duck behavior=" + duckingBehavior + " policy " + pcb.asBinder());
        }
        if (this.mContext.checkCallingPermission(permission.MODIFY_AUDIO_ROUTING) == 0) {
            hasPermissionForPolicy = PREVENT_VOLUME_ADJUSTMENT_IF_SILENT;
        } else {
        ...
        if (hasPermissionForPolicy) {
            synchronized (this.mAudioPolicies) {
                    ...
                    if (duckingBehavior == SENDMSG_NOOP) {
                        for (AudioPolicyProxy policy : this.mAudioPolicies.values()) {
                            if (policy.mFocusDuckBehavior == SENDMSG_NOOP) {
                                Slog.m13e(TAG, "Cannot change audio policy ducking behavior, already handled");
                ...
                Slog.m13e(TAG, "Cannot change audio policy focus properties, unregistered policy");
                return SCO_MODE_UNDEFINED;
            }
        }
        Slog.m19w(TAG, "Cannot change audio policy ducking handling for pid " + Binder.getCallingPid() + " / uid " + Binder.getCallingUid() + ", need MODIFY_AUDIO_ROUTING");
        return SCO_MODE_UNDEFINED;
    }
